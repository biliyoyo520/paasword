...
def main():
    ...
    if sig1_match or sig2_match:
        ...
        # 显示与 fake_time 对得上的 sig2 的带框信息（若存在多个派生结果，sig2 对应 derived_results[1]）
        idx_for_sig2 = 1 if len(derived_results) > 1 else 0
        try:
            print_time_and_pw_box("sig2.bin", ts2, actual_fake, derived_results[idx_for_sig2]["hex_out"], derived_results[idx_for_sig2]["final_pw"])
        except Exception:
            pass
        # 框外：另一个文件信息与密码
    else:
...
def sign_with_time(chpath, outfn, fake_time, gpg_homedir=None, signer_keyid=None):
    ...
    if os.path.exists(outfn):
        os.remove(outfn)
    # Build gpg command; prefer explicit --homedir when provided
    if gpg_homedir:
        cmd = ["gpg", "--homedir", gpg_homedir, f"--faked-system-time={fake_time}", "--digest-algo", "SHA256", "--detach-sign", "--output", outfn]
        if signer_keyid:
            cmd += ["--local-user", signer_keyid]
        ...
        cmd = ["gpg", f"--faked-system-time={fake_time}", "--digest-algo", "SHA256", "--detach-sign", "--output", outfn, chpath]
